...
def plotperf():
    ...
    # Method 2
    total_durations_per_design = defaultdict(lambda: defaultdict(int))
    ...
    for design_name in design_names:
        for num_instrs_id, num_instrs in enumerate(nums_instrs):
            total_durations_per_design[design_name][str(num_instrs)] = sum(instance_gen_durations[design_name][str(num_instrs)]) + sum(run_durations[design_name][str(num_instrs)])
            summed_effective_num_instructions[design_name][str(num_instrs)] = sum(effective_num_instructions[design_name][str(num_instrs)])
            # print (f"{design_name} {num_instrs} {summed_effective_num_instructions[design_name][str(num_instrs)]} {total_durations_per_design[design_name][str(num_instrs)]}")

    # Compute the average throughput
    for design_name in design_names:
        for num_instrs_id, num_instrs in enumerate(nums_instrs):
            instrs_per_second_livegen_per_design[design_name][str(num_instrs)] = summed_effective_num_instructions[design_name][str(num_instrs)] / total_durations_per_design[design_name][str(num_instrs)]

    # Print the results
    for design_name in design_names:
        for num_instrs_id, num_instrs in enumerate(nums_instrs):
            print(f"{design_name} {num_instrs} {int(instrs_per_second_livegen_per_design[design_name][str(num_instrs)])} {int(total_durations_per_design[design_name][str(num_instrs)])}")
    ...
    ax.set_ylabel('Fuzz. instrs/s')
    ax.set_xlabel('Number of fuzzing instructions per program (program length)')
    ax.set_xticks(x)
    ax.set_xticklabels(nums_instrs_strs)
    ax.legend(ncol=2, framealpha=1, loc='lower right')
    plt.tight_layout()
    plt.savefig('fuzzperf_programlength.png', dpi=300)
